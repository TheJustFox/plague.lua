...
local getgenv = function()
	return studio and _G or getgenv()
end

local gui_folder = studio and game.Players.LocalPlayer.PlayerGui or game:GetService("CoreGui")

if gui_folder:FindFirstChild("PLAGUEHUB") then
	getgenv().Unload()
end
...
library.sections = {};
library.hue = 0;
library.rgb = Color3.new();

library.theme = 0

local themes = {
	[0] = {
		LIGHT = Color3.new(0.752941, 0.345098, 0.345098), 
		DARK = Color3.new(0.592156, 0.270588, 0.270588),
		REGULAR = Color3.fromRGB(197, 91, 91)
	},
	[1] = {
		LIGHT = Color3.new(0.345098, 0.509804, 0.752941), 
		DARK = Color3.new(0.270588, 0.4, 0.592157),
		REGULAR = Color3.fromRGB(91, 133, 197)
	},
	[2] = {
		LIGHT = Color3.new(0.741176, 0.345098, 0.752941), 
		DARK = Color3.new(0.501960, 0.270588, 0.592156),
		REGULAR = Color3.fromRGB(164, 91, 197)
	}
}

local colorscheme = themes[library.theme]
...
local function draggable(obj,extern,parented)
	...
	obj.InputBegan:Connect(function(input)
		...
		if (input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch) and souid == false and obj.ZIndex >= index then
			souid = true
	...
	local signal
	signal = uis.InputChanged:Connect(function(input)
	...
	table.insert(uis_signals, signal)
end

getgenv().color_picka = {}
function OpenedColor(text,ColourDisplay,Action,def)
	if getgenv().color_picka[text] ~= nil 
		and getgenv().color_picka[text]["COLORPALLETE"] ~= nil then 
		local COLORPALLETE = getgenv().color_picka[text]["COLORPALLETE"]
		local ANIMATEFRAME01 = getgenv().color_picka[text]["ANIMATEFRAME01"]
		local Holder = getgenv().color_picka[text]["Holder"]
	...
	getgenv().color_picka[text] = {}

	local COLORPALLETE = Instance.new("Frame")
	getgenv().color_picka[text]["COLORPALLETE"] = COLORPALLETE
	local ANIMATEFRAME01 = Instance.new("Frame")
	getgenv().color_picka[text]["ANIMATEFRAME01"] = ANIMATEFRAME01
	local Holder = Instance.new("Frame")
	getgenv().color_picka[text]["Holder"] = Holder
	...
	getgenv().color_picka[text]["is_rainbow"] = false
	RAINBOW.Name = "RAINBOW"
	RAINBOW.Parent = S13
	RAINBOW.BackgroundColor3 = Color3.fromRGB(255, 255, 255)
	RAINBOW.BackgroundTransparency = 1.000
	RAINBOW.BorderSizePixel = 0
	RAINBOW.Position = UDim2.new(0.43, 0, 0.730036497, 0)
	RAINBOW.Size = UDim2.new(0, 15, 0, 15)
	RAINBOW.ZIndex = 23
	RAINBOW.Image = "rbxassetid://103239538066033"
	...
	BG.BorderColor3 = colorscheme.REGULAR
	...
	Upper.BorderColor3 = colorscheme.REGULAR
	...
	linedecoupper.BackgroundColor3 = colorscheme.REGULAR
	linedecoupper.BorderColor3 = colorscheme.REGULAR
	...
	RAINBOW.MouseButton1Click:Connect(function() 
		getgenv().color_picka[text]["is_rainbow"] = not getgenv().color_picka[text]["is_rainbow"]
		ANIMATEFRAME01:TweenPosition(UDim2.new(-0.026,0,-0.007,0),'Out','Quint',0.2,true);
		wait(.2)
		COLORPALLETE.BackgroundTransparency = 1;
		Holder.Visible = false
	...
	local signal
	signal = uis.InputEnded:Connect(function(input)
...
-- BG.Image = "rbxassetid://5991567855" -- ALT: 125143995081140
...
Upper.BorderColor3 = colorscheme.REGULAR
Upper.BorderSizePixel = 0
Upper.AnchorPoint = Vector2.new(1,0)
Upper.Position = UDim2.fromScale(1,0)
Upper.AutomaticSize = Enum.AutomaticSize.X
Upper.Size = UDim2.new(0, 0, 0, 23)
Upper.ZIndex = 2
...
DOWNER.BorderColor3 = colorscheme.REGULAR
...
local stuff = {
	"also try raycast.xyz!",
	"\"mtc v3 will come out\" - mike",
	"dang, i hope no trademark on plaguecheats"
}
WEBSITE.Text = stuff[math.random(1, #stuff)]
...
linedecoDOWNER.BackgroundColor3 = colorscheme.REGULAR
linedecoDOWNER.BorderColor3 = colorscheme.REGULAR
...
icon.Image = "rbxassetid://125143995081140" -- ALT: 8736995700
icon.ZIndex = 3
...
UPPERLABEL.Position = UDim2.new(0, 25, 0, 0)
UPPERLABEL.Size = UDim2.new(0, 0, 0, 23)
UPPERLABEL.ZIndex = 3
UPPERLABEL.Font = Enum.Font.SourceSansSemibold
UPPERLABEL.Text = "plague.lua"
UPPERLABEL.TextColor3 = Color3.fromRGB(199, 199, 199)
UPPERLABEL.TextSize = 17.000
UPPERLABEL.TextXAlignment = Enum.TextXAlignment.Left
...
if not s or e then
	...
	table.insert(uis_signals, game:GetService("RunService").RenderStepped:Connect(function()
		local plr = game:GetService('Players').LocalPlayer
		library.ms =  game:GetService("Stats").Network.ServerStatsItem["Data Ping"]:GetValueString()
	end))
end

table.insert(uis_signals, game:GetService("RunService").RenderStepped:Connect(function(delta)
	library.hue = (library.hue + delta * 0.1) % 1
	library.rgb = Color3.fromHSV(library.hue, 1, 1)
end))
...
function library:AddWatermark(Text)
	...
	obj1.Position = UDim2.new(0, 50, 1, -50)
	...
	obj2.Color = colorscheme.REGULAR
...
function library:AddWindow(text)
	...
	function sec:AddSection(Texto,side)
		...
		F_line.BackgroundColor3 = colorscheme.REGULAR
		F_line.BorderColor3 = colorscheme.REGULAR
		...
		function inside:AddSlider(Text,Max,Min,def,Action, whole)
			local y = {}
			...
			obj7.Color = ColorSequence.new({ColorSequenceKeypoint.new(0, colorscheme.LIGHT), ColorSequenceKeypoint.new(1, colorscheme.DARK)})
			...
			obj4.FocusLost:Connect(function(n)
				if n then
					...
					st = obj4.Text
					Value = tonumber(obj4.Text)
					Value = whole and math.floor(Value) or Value
					local SliderDef = math.clamp(tonumber(obj4.Text), Min, Max) or math.clamp(50, Min, Max)
					local DefaultScale =  (SliderDef - Min) / (Max - Min)
					obj6.Size = UDim2.fromScale(DefaultScale,1)
					pcall(function()
			...
			obj3.MouseButton1Down:Connect(function()
				...
				moveconnection = mouse.Move:Connect(function()
					obj4.Text = whole and Value or ('%0.2f'):format(Value)
				...
				table.insert(uis_signals, releaseconnection)
				obj4.Text = whole and Value or ('%0.2f'):format(Value)
			end)
			...
			function y:set(val)
				...
				if type(val) == 'number' then
					...
					obj4.Text = whole and val or ('%0.2f'):format(val)
					obj6.Size = UDim2.new(math.clamp(val/(Max-Min), 0, 1), 0, 0, 13)
				end
				pcall(function()
					Action(val)
				end)
		...
		inside.KeybindType = function(callback)
			...
			local UNBIND = Instance.new("Frame")
			local TextLabel_3 = Instance.new("TextLabel")
			local Interactive_3 = Instance.new("TextButton")
			local UICorner_3 = Instance.new("UICorner")
			local ALWAYSOFF = Instance.new("Frame")
			local TextLabel_4 = Instance.new("TextLabel")
			local Interactive_4 = Instance.new("TextButton")
			local UICorner_4 = Instance.new("UICorner")

			--Properties:

			BINDFRAME.Name = "BINDFRAME"
			BINDFRAME.Parent = PCR_1
			BINDFRAME.AnchorPoint = Vector2.new(0, 0)
			BINDFRAME.BackgroundColor3 = Color3.fromRGB(33, 33, 33)
			BINDFRAME.BorderSizePixel = 0
			local mousePos = uis:GetMouseLocation() - Vector2.new(0, game:GetService("GuiService"):GetGuiInset().Y)
			BINDFRAME.Position = UDim2.new(0, mousePos.X, 0, mousePos.Y)
			BINDFRAME.Size = UDim2.new(0, 126, 0, 134)
			BINDFRAME.ZIndex = 2

			linedecoupper.Name = "linedecoupper"
			linedecoupper.Parent = BINDFRAME
			linedecoupper.BackgroundColor3 = Color3.fromRGB(197, 91, 91)
			linedecoupper.BorderColor3 = Color3.fromRGB(197, 91, 91)
			linedecoupper.BorderSizePixel = 0
			linedecoupper.Position = UDim2.new(0, 0, 0, 24)
			linedecoupper.Size = UDim2.new(1, 0, 0, 1)
			linedecoupper.ZIndex = 3

			UPPERLABEL.Name = "UPPERLABEL"
			UPPERLABEL.Parent = BINDFRAME
			UPPERLABEL.BackgroundColor3 = Color3.fromRGB(255, 255, 255)
			UPPERLABEL.BackgroundTransparency = 1.000
			UPPERLABEL.BorderSizePixel = 0
			UPPERLABEL.Position = UDim2.new(0, 5, 0, 0)
			UPPERLABEL.Size = UDim2.new(0, 0, 0, 23)
			UPPERLABEL.ZIndex = 3
			UPPERLABEL.Font = Enum.Font.SourceSansSemibold
			UPPERLABEL.Text = "bind type"
			UPPERLABEL.TextColor3 = Color3.fromRGB(199, 199, 199)
			UPPERLABEL.TextSize = 17.000
			UPPERLABEL.TextXAlignment = Enum.TextXAlignment.Left

			limit1.Name = "limit1"
			limit1.Parent = BINDFRAME
			limit1.AnchorPoint = Vector2.new(0.5, 0)
			limit1.BackgroundColor3 = Color3.fromRGB(255, 255, 255)
			limit1.BackgroundTransparency = 1.000
			limit1.BorderSizePixel = 0
			limit1.Position = UDim2.new(0.5, 0, 0, 28)
			limit1.Size = UDim2.new(1, -5, 1, -25)
			limit1.ZIndex = 5

			UIListLayout.Parent = limit1
			UIListLayout.SortOrder = Enum.SortOrder.LayoutOrder
			UIListLayout.Padding = UDim.new(0, 5)

			HOLD.Name = "HOLD"
			HOLD.Parent = limit1
			HOLD.BackgroundColor3 = Color3.fromRGB(25, 25, 25)
			HOLD.BorderSizePixel = 0
			HOLD.ClipsDescendants = true
			HOLD.LayoutOrder = 1
			HOLD.Position = UDim2.new(0.0430313908, 0, 0, 0)
			HOLD.Size = UDim2.new(1, 0, 0, 21)
			HOLD.ZIndex = 14
			...
			Interactive.Size = UDim2.new(1, 0, 0, 21)
			...
			TOGGLE.Name = "TOGGLE"
			TOGGLE.Parent = limit1
			TOGGLE.BackgroundColor3 = Color3.fromRGB(25, 25, 25)
			TOGGLE.BorderSizePixel = 0
			TOGGLE.ClipsDescendants = true
			TOGGLE.LayoutOrder = 2
			TOGGLE.Position = UDim2.new(0.0430313908, 0, 0, 0)
			...
			TextLabel_2.Size = UDim2.new(1, 0, 0, 19)
			TextLabel_2.ZIndex = 15
			TextLabel_2.Font = Enum.Font.SourceSansBold
			TextLabel_2.Text = "TOGGLE"
			TextLabel_2.TextColor3 = Color3.fromRGB(84, 84, 84)
			TextLabel_2.TextSize = 17.000

			Interactive_2.Name = "Interactive"
			Interactive_2.Parent = TOGGLE
			Interactive_2.BackgroundColor3 = Color3.fromRGB(255, 255, 255)
			Interactive_2.BackgroundTransparency = 1.000
			Interactive_2.BorderSizePixel = 0
			Interactive_2.Size = UDim2.new(1, 0, 0, 21)
			Interactive_2.ZIndex = 20
			Interactive_2.Font = Enum.Font.SourceSans
			Interactive_2.Text = ""
			Interactive_2.TextColor3 = Color3.fromRGB(0, 0, 0)
			Interactive_2.TextSize = 18.000

			UICorner_2.CornerRadius = UDim.new(0, 4)
			UICorner_2.Parent = TOGGLE

			UNBIND.Name = "UNBIND"
			UNBIND.Parent = limit1
			UNBIND.BackgroundColor3 = Color3.fromRGB(25, 25, 25)
			UNBIND.BorderSizePixel = 0
			UNBIND.ClipsDescendants = true
			UNBIND.LayoutOrder = 4
			UNBIND.Position = UDim2.new(0.0430313908, 0, 0, 0)
			UNBIND.Size = UDim2.new(1, 0, 0, 21)
			UNBIND.ZIndex = 14

			TextLabel_3.Parent = UNBIND
			TextLabel_3.BackgroundColor3 = Color3.fromRGB(255, 255, 255)
			TextLabel_3.BackgroundTransparency = 1.000
			TextLabel_3.BorderSizePixel = 0
			TextLabel_3.Size = UDim2.new(1, 0, 0, 19)
			TextLabel_3.ZIndex = 15
			TextLabel_3.Font = Enum.Font.SourceSansBold
			TextLabel_3.Text = "UNBIND"
			TextLabel_3.TextColor3 = Color3.fromRGB(84, 84, 84)
			TextLabel_3.TextSize = 17.000

			Interactive_3.Name = "Interactive"
			Interactive_3.Parent = UNBIND
			Interactive_3.BackgroundColor3 = Color3.fromRGB(255, 255, 255)
			Interactive_3.BackgroundTransparency = 1.000
			Interactive_3.BorderSizePixel = 0
			Interactive_3.Size = UDim2.new(1, 0, 0, 21)
			Interactive_3.ZIndex = 20
			Interactive_3.Font = Enum.Font.SourceSans
			Interactive_3.Text = ""
			Interactive_3.TextColor3 = Color3.fromRGB(0, 0, 0)
			Interactive_3.TextSize = 18.000

			UICorner_3.CornerRadius = UDim.new(0, 4)
			UICorner_3.Parent = UNBIND

			ALWAYSOFF.Name = "ALWAYSOFF"
			ALWAYSOFF.Parent = limit1
			ALWAYSOFF.BackgroundColor3 = Color3.fromRGB(25, 25, 25)
			ALWAYSOFF.BorderSizePixel = 0
			ALWAYSOFF.ClipsDescendants = true
			ALWAYSOFF.LayoutOrder = 3
			ALWAYSOFF.Position = UDim2.new(0.0430313908, 0, 0, 0)
			ALWAYSOFF.Size = UDim2.new(1, 0, 0, 21)
			ALWAYSOFF.ZIndex = 14

			TextLabel_4.Parent = ALWAYSOFF
			TextLabel_4.BackgroundColor3 = Color3.fromRGB(255, 255, 255)
			TextLabel_4.BackgroundTransparency = 1.000
			TextLabel_4.BorderSizePixel = 0
			TextLabel_4.Size = UDim2.new(1, 0, 0, 19)
			TextLabel_4.ZIndex = 15
			TextLabel_4.Font = Enum.Font.SourceSansBold
			TextLabel_4.Text = "ALWAYS OFF"
			TextLabel_4.TextColor3 = Color3.fromRGB(84, 84, 84)
			TextLabel_4.TextSize = 17.000

			Interactive_4.Name = "Interactive"
			Interactive_4.Parent = ALWAYSOFF
			Interactive_4.BackgroundColor3 = Color3.fromRGB(255, 255, 255)
			Interactive_4.BackgroundTransparency = 1.000
			Interactive_4.BorderSizePixel = 0
			Interactive_4.Size = UDim2.new(1, 0, 0, 21)
			Interactive_4.ZIndex = 20
			Interactive_4.Font = Enum.Font.SourceSans
			Interactive_4.Text = ""
			Interactive_4.TextColor3 = Color3.fromRGB(0, 0, 0)
			Interactive_4.TextSize = 18.000

			UICorner_4.CornerRadius = UDim.new(0, 4)
			UICorner_4.Parent = ALWAYSOFF
			
			local hovering = false
			
			local setup = function(button, return_type)
			...
			setup(Interactive, 'HOLD')
			setup(Interactive_2, 'TOGGLE')
			setup(Interactive_3, 'UNBIND')
			setup(Interactive_4, 'ALWAYSOFF')
			
			uis.InputBegan:Once(function(input: InputObject, game_processed: boolean)
		...
		function inside:AddToggle(Text,Enabled,keybind: Enum.KeyCode,Callback, friendly_name)
			...
			local function Update(state)
				...
				if keybind then
					if state then
						bind_list.enable(friendly_name)
					else
						bind_list.disable(friendly_name)
					end
				end
				if activated == true then
					TweenService:Create(color , TweenInfo.new(0.26, Enum.EasingStyle.Quad , Enum.EasingDirection.InOut), {BackgroundColor3 = colorscheme.LIGHT}):Play()
			...
			local keytype = "TOGGLE";
			local KeyCode = keybind;
			
			if keybind then
				friendly_name = friendly_name or Text
				bind_list.add(friendly_name, keybind.Name)
				if Enabled then
					bind_list.enable(friendly_name)
				else
					bind_list.disable(friendly_name)
				...
				local signal
				signal = uis.InputBegan:Connect(function(a, gp)
					if keytype == "ALWAYSOFF" then return end
					if not gp then 
						if (a.KeyCode.Name == KeyCode or a.KeyCode.Name == KeyCode.Name) and ischanging == false and a.KeyCode.Name ~= "Unknown" then 
							local new_value = keytype == "HOLD" and true or not activated
							pcall(function()
								Update(new_value)
							end)
						end
				...
				table.insert(uis_signals, signal)
				signal = uis.InputEnded:Connect(function(a, gp)
					if keytype ~= "HOLD" then return end
					if keytype == "ALWAYSOFF" then return end
					if not gp then 
						if (a.KeyCode.Name == KeyCode or a.KeyCode.Name == KeyCode.Name) and ischanging == false and a.KeyCode.Name ~= "Unknown" then 
							pcall(function()
				...
				KeyButton.MouseButton1Click:connect(function() 
					...
					if v1.KeyCode.Name ~= "Unknown" then
						...
						ischanging = true
						KeyButton:TweenSize(UDim2.new(0,getsize( "None"),0,13), "Out", "Quint", 0.3, true)
						KeyButton.Text =  "None"
						KeyCode = "Unknown";
						wait(.2)
						ischanging = false
					end
					bind_list.update(friendly_name, KeyCode)
				end)
				KeyButton.MouseButton2Click:Connect(function()
					if ischanging then return end
					ischanging = true
					local thingy = function(result)
						ischanging = false
						if result == 'UNBIND' then
							ischanging = true
							KeyButton:TweenSize(UDim2.new(0,getsize("None"),0,13), "Out", "Quint", 0.3, true)
							KeyButton.Text =  "None"
							KeyCode = "Unknown";
			...
			function y:set(args)
				...
				activated = args["state"]
				Update(activated)
				if keybind then
					keytype = args["keytype"] or nil
					KeyCode = Enum.KeyCode[args["keycode"] or "Unknown"] or Enum.KeyCode.Unknown
					if KeyCode.Name ~= "Unknown" then
						game.TweenService:Create(KeyButton, TweenInfo.new(0.1, Enum.EasingStyle.Linear, Enum.EasingDirection.In), {
							BackgroundColor3 = Color3.fromRGB(25, 25, 25)
						}):Play()
						KeyButton:TweenSize(UDim2.new(0,getsize(KeyCode.Name),0,13), "Out", "Quint", 0.3, true)
		...
		function inside:AddColorPallete(Text,Color,Action)
			...
			ColourDisplay.BorderSizePixel = 1
			ColourDisplay.BorderColor3 = colorscheme.REGULAR
			ColourDisplay.Position = UDim2.new(0, 235, 0, 4)
			ColourDisplay.Size = UDim2.new(0, 14, 0, 14)
			...
			function y:set(val)
				if getgenv().color_picka[Text] == nil then
					getgenv().color_picka[Text] = {}
				end
				if val == "rgb" and not getgenv().color_picka[Text]["is_rainbow"] then
					getgenv().color_picka[Text]["is_rainbow"] = true
					val = library.rgb
				end
				ColourDisplay.BackgroundColor3 = val
				task.spawn(Action,val)
			end

			table.insert(uis_signals, RunService.RenderStepped:Connect(function()
				if getgenv().color_picka[Text] and getgenv().color_picka[Text]["is_rainbow"] then
					--ColourDisplayBIG.ImageColor3 = library.rgb
					--UIGradient.Color = ColorSequence.new{
					--	ColorSequenceKeypoint.new(0, library.rgb), 
					--	ColorSequenceKeypoint.new(1, Color3.new(0, 0, 0))
					--}
					ColourDisplay.BackgroundColor3 = library.rgb
					Action( library.rgb )
				end
			end))
		...
		function inside:AddKeyBind(Text,KeyCode,Action)
			...
			internal.SetKeycode = function(v1)
				ischanging = true
				game.TweenService:Create(KeyButton, TweenInfo.new(0.1, Enum.EasingStyle.Linear, Enum.EasingDirection.In), {
					BackgroundColor3 = Color3.fromRGB(25, 25, 25)
				}):Play()
...
local bindlist = function()
	...
	y.add = function(text, bind)
		if stuff[text] ~= nil then return end
		local BINDTHING = Instance.new("Frame")
		local TextLabel = Instance.new("TextLabel")

		BINDTHING.Name = "thing"
		BINDTHING.Parent = limit1
		BINDTHING.BackgroundColor3 = Color3.fromRGB(25, 25, 25)
		BINDTHING.BorderSizePixel = 0
		BINDTHING.ClipsDescendants = true
		BINDTHING.LayoutOrder = 3
		BINDTHING.Position = UDim2.new(0.0430313908, 0, 0, 0)
		BINDTHING.Size = UDim2.new(1, 0, 0, 21)
		BINDTHING.ZIndex = 14

		TextLabel.Parent = BINDTHING
		TextLabel.BackgroundColor3 = Color3.fromRGB(255, 255, 255)
		TextLabel.BackgroundTransparency = 1.000
		TextLabel.BorderSizePixel = 0
		TextLabel.Size = UDim2.new(1, 0, 0, 19)
		...
		TextLabel.Text = bind .. " // " .. text
		TextLabel.TextColor3 = Color3.fromRGB(84, 84, 84)
		TextLabel.TextSize = 17.000

		stuff[text] = BINDTHING
	end
	y.enable = function(text)
		if stuff[text] == nil then return end
		stuff[text].TextLabel.TextColor3 = Color3.fromRGB(199, 199, 199)
	end
	y.disable = function(text)
		if stuff[text] == nil then return end
		stuff[text].TextLabel.TextColor3 = Color3.fromRGB(84, 84, 84)
	end
	y.update = function(text, new_bind)
		stuff[text].TextLabel.Text = new_bind .. " // " .. text
	end

	--Properties:

	BINDLIST.Name = "BINDLIST"
	BINDLIST.Parent = PCR_1
	BINDLIST.AnchorPoint = Vector2.new(0.5, 0.5)
	BINDLIST.BackgroundColor3 = Color3.fromRGB(33, 33, 33)
	BINDLIST.BorderSizePixel = 0
	BINDLIST.Position = UDim2.new(0.3, 0, 0.5, 0)
	BINDLIST.Size = UDim2.new(0, 250, 0, 0)
	BINDLIST.AutomaticSize = Enum.AutomaticSize.Y
	BINDLIST.ZIndex = 2

	linedecoupper.Name = "linedecoupper"
	linedecoupper.Parent = BINDLIST
	linedecoupper.BackgroundColor3 = Color3.fromRGB(197, 91, 91)
	linedecoupper.BorderColor3 = Color3.fromRGB(197, 91, 91)
	linedecoupper.BorderSizePixel = 0
	linedecoupper.Position = UDim2.new(0, 0, 0, 24)
	...
	UPPERLABEL.ZIndex = 3
	UPPERLABEL.Font = Enum.Font.SourceSansSemibold
	UPPERLABEL.Text = "binds"
	UPPERLABEL.TextColor3 = Color3.fromRGB(199, 199, 199)
	UPPERLABEL.TextSize = 17.000
	UPPERLABEL.TextXAlignment = Enum.TextXAlignment.Left

	limit1.Name = "limit1"
	limit1.Parent = BINDLIST
	limit1.AnchorPoint = Vector2.new(0.5, 0)
	limit1.BackgroundColor3 = Color3.fromRGB(255, 255, 255)
	limit1.BackgroundTransparency = 1.000
	limit1.BorderSizePixel = 0
...
library.GUI = PCR_1
library.bindlist = bind_list

function library.Unload()
	for _, signal in pairs(uis_signals) do
		print(signal)
		print(_)
		signal:Disconnect()
	end
